...
> _(OLAP)_ в режиме реального времени.  
...
- [Руководство по установке и настройке ClickHouse](clickhouse/ch_install.md)
...
В **ClickHouse** используются различные [движки таблиц](clickhouse/ch_engines.md), определяющие способы хранения, 
...
[Сегментация и партиционирование](clickhouse/ch_shard_part.md) — ключевые методы организации хранения и обработки 
больших объемов данных в аналитических системах **ClickHouse**.  

**ClickHouse** поддерживает **сегментацию** *(шардирование, sharding)*, позволяющую разделять данные на независимые сегменты, 
которые могут храниться и обрабатываться на различных физических или логических узлах кластера.  

**ClickHouse** поддерживает и **партиционирование** *(partitioning)*  таблиц, позволяющее логически разбивать данные на 
независимые части *(partition)*. Это упрощает управление данными, ускоряет удаление/архивацию и повышает 
...
В аналитических системах хранения данных [индексы](clickhouse/ch_indexes.md) играют ключевую роль в оптимизации 
выполнения запросов, минимизации времени отклика и эффективном использовании ресурсов хранения.  

**Primary Key** в **ClickHouse** не обеспечивает уникальность строк (используется для организации данных и ускорения поиска).  

**Primary Key** определяет порядок хранения строк внутри частей и создаёт _Sparse Index_ — разреженный индекс, который указывает 
на блоки данных.  
...
Колоночное хранение в **ClickHouse** позволяет применять эффективные [алгоритмы сжатия](clickhouse/ch_compression.md), 
такие как *LZ4*, *ZSTD*, *Deflate*.  
...
**ClickHouse** периодически объединяет маленькие части данных в более крупные, оптимизируя хранение и производительность. 
...
В **ClickHouse** реализация [жизненного цикла данных](clickhouse/ch_lifecycle.md) оптимизирована для работы с большими 
объёмами информации и высокой скоростью аналитических запросов.  

### Основные этапы жизненного цикла данных
1. Получение и загрузка данных *(Data Ingestion)*.  
2. Преобразование и очистка данных *(Data Transformation and Cleansing)*.  
3. Хранение и организация данных *(Data Storage and Organization)*.  
4. Обеспечение доступности и управления данными *(Data Accessibility and Management)*.  
5. Анализ и использование данных *(Data Analysis and Utilization)*.  
6. Архивирование и удаление данных *(Data Archiving and Deletion)*.  

## 6.2.12 Оптимизация SQL-запросов в ClickHouse
> **Оптимизация SQL-запросов** — совокупность методов и стратегий, направленных на повышение производительности 
> выполнения запросов в СУБД.  

[Оптимизация включает анализ структуры запросов](clickhouse/ch_query_optima.md), правильный выбор типов данных 
и индексов, эффективное использование архитектурных особенностей **ClickHouse**, снижение объёма обрабатываемых 
данных, а также применение специальных функций и операторов языка **SQL**.  
