...
В **CH** используются различные [движки таблиц](clickhouse/ch_engines.md), определяющие способы хранения, 
...
**CH** поддерживает **сегментацию** *(шардирование, sharding)*, позволяющую разделять данные на независимые сегменты, 
которые могут храниться и обрабатываться на различных физических или логических узлах кластера.  

**CH** поддерживает и **партиционирование** *(partitioning)*  таблиц, позволяющее логически разбивать данные на 
...
Колоночное хранение в **CH** позволяет применять эффективные [алгоритмы сжатия](clickhouse/ch_compression.md), 
