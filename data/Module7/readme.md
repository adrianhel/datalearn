...
**Spark Session** позволяет работать с различными типами данных, включая **RDD**, **DataFrames** и **Datasets**, а 
также выполнять запросы на языке **SQL**, манипулировать структурированными данными и выполнять аналитику в реальном 
времени.  
...
**Driver** – это и есть наша **Spark Session**, в ней задается конфигурация запуска нашего приложения и многое 
другое.  
...
Исполнители нужны для выполнения задач (одной или нескольких) Spark. Именно в исполнителях (экзекьюторах) происходит 
обработка данных, они работают параллельно и потом отдают результат драйверу.  

Есть **Worker Node**, которые задаются **Cluster Manager** динамически, исходя из этих **Worker Node** и доступной 
памяти, мы запускаем приложение Spark.  
  
На одной **Worker Node** может находиться несколько **Executors**, если будет хватать ресурсов.  
Также в **Executors** может находиться несколько задач.  

## 7.6 Структуры данных в Apache Spark
В Spark два основных абстрактных уровня работы с данными в распределенных вычислениях:
...
Очевидно, что высокоуровневые структуры имеют ряд [преимуществ](data/spark_advantages_over_rdd.md) над RDD.

## 7.7 Преимущества Spark для аналитики
...
> Ленивые вычисления — это "двигатель", преобразования — "маршрут на карте", а действия — 
> это команда "поехали!", которая заставляет двигатель работать и проходить проложенный маршрут.

## 7.9 Spark SQL
[Spark SQL](data/spark_sql.md) — компонент для обработки структурированных данных. 
Предоставляет интерфейс для работы с данными в виде таблиц, поддерживает запросы на языке SQL, а также интеграцию 
с программными интерфейсами на языках Scala, Java, Python и R.  
   
> Spark SQL оптимизирован для масштабируемой обработки больших объёмов данных и является основным инструментом для 
аналитики в рамках экосистемы Spark.  

## 7.10 Spark Streaming
[Spark Streaming](data/spark_streaming.md) — компонент для потоковой обработки данных в реальном времени.  
Расширяет архитектуру Spark, позволяя анализировать непрерывные потоки входящих данных, обрабатывая их в виде небольших 
пакетов (batches) с низкой задержкой.  
...
## 7.11 Spark MLlib
[Spark MLlib](data/spark_ml.md) — библиотека машинного обучения.  
Предоставляет высокоуровневые API для создания, обучения и применения моделей машинного обучения на больших 
...
## 7.12 Spark GraphX
[Spark GraphX](data/spark_graphx.md) — библиотека для параллельной обработки и анализа графов.  
...
Spark обладает [рядом ограничений](data/spark_limits.md) и особенностей, критичных для проектирования и эксплуатации 
