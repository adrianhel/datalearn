...
- **Пример агрегации с использованием DataFrame:**  

  ```python
    df.groupBy("department").agg({"salary": "avg", "age": "max"})
  ```

- **Пример эквивалентной операции с использованием RDD:**  

  ```python
  rdd.map(lambda row: (row.department, (row.salary, row.age)))
    .aggregateByKey((0, 0, 0), 
                   lambda acc, value: (acc[0] + value[0], max(acc[1], value[1]), acc[2] + 1), 
                   lambda acc1, acc2: (acc1[0] + acc2[0], max(acc1[1], acc2[1]), acc1[2] + acc2[2]))
    .mapValues(lambda acc: (acc[0] / acc[2], acc[1]))
  ```

- **Сравнение:**  
    - Операция на DataFrame существенно проще и короче, а оптимизации производятся автоматически.  
      - Эквивалентная операция на RDD требует ручной реализации агрегации и объединения промежуточных результатов.  

### 10. Ограничения RDD по сравнению с DataFrame
- **Отсутствие оптимизации запросов:** RDD не позволяет применять логические и физические оптимизации, такие 
как перестановка фильтров или оптимизация планов выполнения.  
- **Нет поддержки схемы:** Работа с неструктурированными данными требует ручного управления типами и структурой данных.  
- **Более низкая производительность:** Все операции выполняются в виде пользовательских функций, что затрудняет 
оптимизацию и приводит к большему расходу ресурсов.  
- **Сложность интеграции:** RDD не интегрируется напрямую с SQL и внешними источниками данных с поддержкой схемы.  
  
### 11. Заключительная сравнительная таблица

| Критерий                               | DataFrame                    | RDD                                       |
|----------------------------------------|------------------------------|-------------------------------------------|
| ***Оптимизация***                      | Catalyst Optimizer, Tungsten | Отсутствует                               |
| ***Схема данных***                     | Явная, структурированная     | Не требуется, неструктурированные объекты |
| ***API***                              | Декларативный, SQL-подобный  | Императивный, функциональный              |
| ***Производительность***               | Высокая, за счет оптимизаций | Средняя, отсутствуют оптимизации          |
| ***Интеграция с SQL***                 | Да                           | Нет                                       |
| ***Поддержка внешних источников***     | Да, с оптимизациями          | Ограниченная, без схемы                   |
| ***Применение аналитических функций*** | Да, встроено                 | Требуется ручная реализация               |
